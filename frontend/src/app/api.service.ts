...
export interface Transaction { id: number; transactionType: 'DEPOSIT' | 'WITHDRAW' | 'TRANSFER'; amount: number; senderAccountNumber: string | null; receiverAccountNumber: string | null; transactionTime: string; status: string }
...
export class ApiService {
  ...
  money(accountId: number, action: 'deposit' | 'withdraw' | 'transfer', amount: number, receiveAccountId?: number) {
    return this.http.post<Account>(`/api/account/${accountId}/${action}`, action === 'transfer' ? { amount, receiveAccountId } : { amount });
  }
}
