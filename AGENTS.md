...
<<<<<<< ours
>>>>>>> theirs
# Banking System agent guide

## Scope

These instructions apply to this repository. Treat it as a banking application: correctness, authorization, auditability, and monetary precision take priority over convenience.

## Project map

- Backend: Java 17, Spring Boot 3.5, Maven wrapper (`./mvnw`).
- Frontend: Angular 17 in `frontend/`.
- Persistence: MySQL with Flyway migrations in `src/main/resources/db/migration/`.
- Cache: Redis.
- Architecture: modular monolith under `com.banking`; keep business capabilities separated by package.
- Supporting design documents live in `Docs/` and the root `README.md`.

## Working rules

- Read the affected module, its tests, and relevant documentation before changing behavior.
- Keep controllers thin. Put business rules and transaction boundaries in the application/service layer.
- Do not bypass module boundaries or introduce dependencies from one business module's internals into another.
- Represent money with `BigDecimal`; never use `float` or `double`. State scale and rounding behavior when division is involved.
- Preserve transaction atomicity, deterministic account-lock ordering, and immutable ledger history.
- Enforce ownership and authorization server-side. Never log passwords, JWTs, secrets, or sensitive banking data.
- Make schema changes only through a new Flyway migration. Never edit an already-applied migration.
- Do not place credentials in tracked files. Production and local-secret property files remain ignored.
- Prefer the smallest change that satisfies the requirement; avoid unrelated refactors.

## Verification

- Backend tests: `./mvnw test`
- Single backend test: `./mvnw -Dtest=ClassName test`
- Backend package: `./mvnw clean verify`
- Frontend build: `npm --prefix frontend run build`
- Local services: `docker compose up -d mysql redis`

Add or update tests for every behavior change. For money movement, security, persistence, or concurrency changes, include regression tests covering failure and authorization paths, not only the happy path.

## Completion checklist

- Relevant tests pass.
- Backend changes respect module boundaries and existing API/error conventions.
- Financial operations remain atomic and use exact decimal arithmetic.
- Security-sensitive changes include negative authorization tests.
- Documentation is updated when API, configuration, schema, or operating steps change.
